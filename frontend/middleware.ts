...
import { requiredPermissionFor } from "@/lib/rbac/navigation";
...
export function middleware(req: NextRequest) {
  ...
  // Role gate, from the same table the sidebar filters on.
  const permission = requiredPermissionFor(pathname);
  if (permission && claims && !hasPermission(claims.role, permission)) {
