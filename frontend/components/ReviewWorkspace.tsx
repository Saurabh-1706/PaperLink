...
import type { GradingSummary, MappedAnswer, PageImage, Question } from "@/lib/types";

type StatusFilter = "all" | "correct" | "partial" | "incorrect" | "unanswered";
...
export default function ReviewWorkspace({
  ...
  const byQuestion = new Map(mappings.map((m) => [m.questionId, m]));
  ...
  const filtered = questions.filter((q) => {
    const matchesSearch =
      q.text.toLowerCase().includes(search.toLowerCase()) || `question ${q.number}`.toLowerCase().includes(search.toLowerCase());
  ...
  const selectedMapping: MappedAnswer | null =
    selectedUnmatchedIdx !== null ? unmatched[selectedUnmatchedIdx] : (selectedId && byQuestion.get(selectedId)) || null;
